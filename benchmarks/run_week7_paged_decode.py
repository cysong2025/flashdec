"""Run Week 7 paged decode attention shape sweep benchmarks."""
...
def _make_shape_matrix(args):
    ...
    if args.quick:
        ...
        batches = BATCH_SWEEP
        contexts = CONTEXT_SWEEP

    for batch in batches:
        shapes.append((batch, args.num_q_heads, args.num_kv_heads, args.head_dim, args.batch_context))
    for context in contexts:
        shapes.append((args.context_batch, args.num_q_heads, args.num_kv_heads, args.head_dim, context))
...
def _validate(torch, q, k_cache, v_cache, block_tables, seq_lens, block_size, num_warps):
    ...
    actual = paged_decode_attention(q, k_cache, v_cache, block_tables, seq_lens, block_size=block_size, num_warps=num_warps)
...
def run_shape(torch, args, shape, dtype):
    ...
    if args.mode == "all":
        ref_result = benchmark_case(
            ...
            warmup=args.warmup,
            repeat=args.repeat,
            metadata=_common_metadata(torch, dtype_name, "torch_ref", shape, seq_lens, block_tables, cache, args.block_size),
        )
    ...
    triton_result = benchmark_case(
        ...
        repeat=args.repeat,
        metadata={
            **_common_metadata(torch, dtype_name, "triton", shape, seq_lens, block_tables, cache, args.block_size),
            "num_warps": args.num_warps,
        },
    )
    if ref_mean_ms is None:
        results.append(triton_result)
    else:
        results.append(_with_speedup(triton_result, ref_mean_ms))
    return results
...
def parse_args():
    ...
    parser.add_argument("--mode", choices=["all", "triton"], default="all")
    ...
    parser.add_argument("--output", default="benchmarks/results/week7_paged_decode.csv")
...
def main():
    ...
    results = []
    shapes = _make_shape_matrix(args)
    for dtype in dtypes:
        for shape in shapes:
