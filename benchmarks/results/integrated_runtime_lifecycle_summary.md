...
- This matrix reports one integrated workload; it is not a shared-prefix speedup A/B and does not reopen kernel configuration selection.
...
- 该实验验证组合 correctness 与 lifecycle closure，而不是相对 latency ratio；24 行均通过 strict validator。
...
- 该矩阵不比较 persistent-metadata candidate，也不证明 shared prefix 带来 latency speedup；materialized transaction metadata 保持默认。
