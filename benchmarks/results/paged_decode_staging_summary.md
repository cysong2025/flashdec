# Paged Decode Triton Staging Sweep Summary
...
因此保留 `num_stages=None`，让 Triton 使用 implicit default；不增加显式 staging 默认值、shape dispatch 或 dtype dispatch。通用 kernel 配置选择为 token-major、`block_size=32`、`num_warps=2`、implicit staging。
