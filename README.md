...
> 当前状态：R1 Block-aware Scheduler 与 R2 Multi-layer KV Token Transaction 已完成设计、correctness、RTX 5070 正式实验和文档闭环。R3-A Cache ownership core 与 R3-B DecodeEngine/scheduler integration 已通过 WSL focused/full 回归；R3-C RTX quick 的 4 行矩阵已通过严格校验，FP16/BF16 三轮正式矩阵待执行。仓库仍处于 `0.0.0`，clean-install 与正式版本发布留在最终 release gate。
...
| R3-C RTX quick | 75% hit：context `2/4` blocks、peak `6` vs 0% 的 `8` | 单轮只验证内存/accounting 链路，不形成 latency 结论 |
...
详细结果见[性能报告](docs/performance_report.md)、[R1 正式摘要](benchmarks/results/r1_scheduler_workload_trials3_summary.md)和[R2 正式摘要](benchmarks/results/r2_multi_layer_engine_trials3_summary.md)。R3-C quick 的精确内存结果记录在[Shared Prefix Blocks 设计](docs/design_shared_prefix_blocks.md)，正式摘要将在三轮矩阵完成后归档。
