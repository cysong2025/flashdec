...
| multi-layer KV | Cache transaction + Engine sequential layer API + fused location-only write | R2 correctness 与 144 行正式性能证据已完成 |
...
- Multi-layer KV Token Transaction 已完成 Cache reference、Engine sequential layer API、fused CUDA location-only write 与正式 RTX 144 行矩阵。融合路径的主要收益来自 append 与 launch 减少；20-repeat 下的 p99 接近单轮最大值，只用于暴露离群点，不作为稳定生产长尾结论。
