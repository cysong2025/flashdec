...
- profiler 使用独立 WARMUP→active schedule；warmup token abort，不进入 active evidence。每个 active layer 必须有精确一个 CPU user annotation，其 inclusive CPU time 保留 checked 路径子 `.item()` 等待，其 canonical correlated `device_time_total` 提供设备归因。CUDA activity count排除 user annotations；range/device/scalar count 任一不完整都重建整个 probe，最多三次并记录 attempt count，不能接受缺 range 或手工补值。
...
修复后的 runner 用 profiler schedule 先执行并 abort 一个同形 warmup token，再在唯一 active window 采集原 `profile_steps`；严格验证每个 CPU range 的 inclusive host、canonical correlated device time，以及 checked/trusted 预期的 scalar extraction count，CUDA activity count 排除 user annotations。仅 trace completeness 错误允许全新 engine/profiler 以相同 seed 最多重采集三次，attempt count进入 CSV；业务错误立即透传。paired trial 的两条正式 wall 都先于 attribution 完成，重采集不会插入 wall pair 中间。先通过 l4 stress quick，再重启 5-trial/160-row 正式矩阵。R4-A 尚未冻结，transaction metadata reuse仍是后续独立 slice。
