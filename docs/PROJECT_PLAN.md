...
| Shared Prefix R3 | immutable full-block reuse、refcount/LRU、shared-aware admission | R3-A/R3-B WSL 回归与 R3-C RTX quick 已通过；正式矩阵待完成 |
...
- R3-A shared-prefix Cache ownership 与 R3-B Engine/scheduler integration 已通过 WSL focused/full 回归。R3-B focused 为 `56 passed, 14 subtests passed in 5.29s`，完整回归为 `352 passed, 25 subtests passed in 9.37s`。R3-C commit `fd36ed0` 的 RTX quick 4 行矩阵已通过严格校验；75% hit 将 context physical blocks 从 `4/4` 降至 `2/4`，FP16/BF16 三轮正式矩阵待完成。
