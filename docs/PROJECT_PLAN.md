...
| Trusted Transaction R4 | checked public raw op、Cache-owned device-value-check-free path、persistent metadata、统一多层调度证据 | R4-A 在 `d25107f` 冻结；R4-B core/paired evidence tooling 已实现，等待 RTX correctness、quick/formal，尚无性能结论 |
...
- R4-A 针对 multi-layer fused transaction 每 layer 重复执行的 CUDA index reduction + `.item()` host sync。public raw primitive 保持完整检查；`PagedKVCache.begin_token()` 以纯 host invariant 证明 allocator 位置，公开 transaction API 根据 transaction id 回查该内部状态并调用 private trusted raw primitive。benchmark commit `4018449` 已在 RTX 5070 完成 focused `73 passed, 23 subtests passed`、full `410 passed, 48 subtests passed` 与 160-row/80-pair 五轮正式矩阵。trusted complete-token p50/TPS 几何平均为 `1.7307x/1.7131x`，16/16 个 dtype/case p50 ranges 均稳定胜出，append inclusive CPU 为 `2.3612x`；7/16 个 p99 ranges跨 1，因此只冻结 p50 与 host-sync 归因，不声明稳定尾延迟或 device-kernel 加速。R4-A 已在 commit `d25107f` 冻结。
- R4-B 已实现五 tensor private canonical bundle、transaction 发布前原子 snapshot、public no-alias、Engine private math routing、terminal device metadata release，以及 build/materialization/reuse/release/resident counters与invariant。paired runner在同一commit内用benchmark-only hooks恢复legacy `2L+2` Cache views；persistent路径为1次materialization加`L`次reuse，两侧都使用R4-A trusted raw math。Mac完成16个dependency-free tests、`py_compile`和diff check；因本机没有torch/pytest，RTX correctness、quick/formal待执行，不能把实现就绪写成R4-B完成或加速。
