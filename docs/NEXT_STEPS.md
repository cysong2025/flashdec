...
当前进入 R2-D multi-layer workload。R2-A Cache reference transaction 已通过 WSL `313 passed, 20 subtests passed` 完整回归。R2-B Engine sequential API 已在 commit `a009b45` 通过 focused `71 passed, 8 subtests passed` 和完整回归 `322 passed, 20 subtests passed`。R2-C fused CUDA location-only transaction write 已在 commit `6afc89f` 通过 RTX 5070 focused `131 passed in 6.21s` 和完整回归 `326 passed, 20 subtests passed in 6.23s`，摘要无 skipped、warning 或 failure。下一步固定 multi-layer workload 的 layer 数、dtype、batch/context、trial/seed 和计时边界，分离 complete-token、per-layer device、allocator/preflight/commit 成本，并记录 KV bytes、launch、transaction 与 rollback 指标。clean-machine install 继续推迟到全部主线功能完成后的最终发布验证，期间版本保持 `0.0.0`。
