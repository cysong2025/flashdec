...
当前 PyTorch RoPE + paged KV append reference 和统一返回接口已通过 RTX 5070 correctness。独立 CUDA KV append 的 JIT extension、raw op 与 runtime integration 也已通过 RTX 5070 验证（focused `34 passed in 3.59s`，full `198 passed in 5.13s`）；现已实现可选 `append_backend="cuda"` 的 RoPE integration，等待其 GPU correctness。benchmark 与 RoPE fusion 仍待实现。
...
| Decode Data Path | PyTorch reference 与独立 CUDA KV append 已通过 RTX correctness；RoPE native append integration 已实现 | integration correctness、RoPE fusion、native path benchmark |
