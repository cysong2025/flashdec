...
@pytest.mark.skipif(not torch.cuda.is_available(), reason="CUDA GPU is required for Triton kernel tests")
def test_decode_infers_block_size_from_cache(block_size):
    q = torch.randn((1, 2, 64), device="cuda", dtype=torch.float16)
    ...
    actual = flashdec.decode(q, k_cache, v_cache, block_tables, seq_lens)
    expected = flashdec.paged_decode_attention_ref(q, k_cache, v_cache, block_tables, seq_lens)
